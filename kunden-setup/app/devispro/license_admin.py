...
def kunde_anlegen(kunde_id, firma, email, pilot=False):
    db = _laden()
    heute = dt.date.today()
    if pilot:
        ...
        betrag = PREIS_EINRICHTUNG + PREIS_JAHR
    db[kunde_id] = {
    ...
    _speichern(db)
    if not pilot:
        rechnung_erstellen(kunde_id, betrag, "Erstlizenz")
    ...
    liz._schreibe_lizenz(kunde_id, liz_bis, erst_code)
    return db[kunde_id]
...
def trial_anmelden(daten: dict) -> dict:
    ...
    if kunde_id in db and db[kunde_id].get("bezahlt"):
        ...
        k = kunde_anlegen(kunde_id, firma, email, pilot=True)
        bis = k["gueltig_bis"]
...
def rechnung_erstellen(kunde_id, betrag, typ="Jahresverlaengerung"):
    db = _laden()
    k = db.get(kunde_id)
    if not k:
...
<tr><td>{typ} DevisPro</td><td style="text-align:right">{betrag:,.2f} CHF</td></tr>
</table>
...
def freigeben_nach_zahlung(kunde_id, auto_versenden=True):
    ...
    _speichern(db)
    # Rechnung fuer die Verlaengerung
    rechnung_erstellen(kunde_id, PREIS_JAHR, "Jahresverlaengerung")
    mail_ok = False
    if auto_versenden:
