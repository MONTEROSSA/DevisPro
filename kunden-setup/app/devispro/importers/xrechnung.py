...
class XRechnungImporter(BaseImporter):
    ...
    def parse(self, path: str) -> Devis:
        ...
        for elem in root.iter():
            ...
            if tag in ("lineitem", "item", "invoicedline", "supplychainnettradelineitem"):
                text = ""
                menge = 0.0
                einheit = ""
                ep = None
                ...
                for child in elem.iter():
                    ...
                    if ct in ("description", "name", "itemname"):
                        text = (child.text or "").strip() or text
                    elif ct in ("quantity", "billedquantity"):
                        try:
                            menge = float((child.text or "0").replace(",", "."))
                        except ValueError:
                            menge = 0.0
                    elif ct in ("unitcode", "unit"):
                        einheit = (child.text or "").strip()
                    elif ct in ("priceamount", "netprice"):
                        try:
                            ep = float((child.text or "0").replace(",", "."))
                        except ValueError:
                            ep = None
                    elif ct in ("lineextensionamount", "linetotal"):
                        try:
