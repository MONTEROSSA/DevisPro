...
def import_ausschreibung(pfad: str, kanton: str = "ZH", stichwort: str = ""):
    ...
    if not rows:
        # ohne eigene Preisliste: Demo-Preise, damit der Flow sofort funktioniert
        from devispro.lead_magnet import demo_pricelist
        d = demo_pricelist()
        rows = [v for v in d.values()] if isinstance(d, dict) else d
