...
def detect_importer(path: str):
    ...
    # .crbx: modern = CRB-XML, alt = ZIP mit SIAFILE.e1s -> inhalt pruefen
    if ext in ("crbx", "crb"):
        try:
            import zipfile
            with open(path, "rb") as f:
                head_bytes = f.read(4)
            if head_bytes[:2] == b"PK":           # echtes ZIP -> SIA-e1s Parser
                from .crbx_sia import CrbxSiaImporter
                return CrbxSiaImporter
            # FIX: klassisches Fixed-Width-CRB (Sorba) erkennen! Dateien, die
            # mit Record-Typ '01'/'11' beginnen (Text, keine '<'), sind KEIN
            # XML und gehoeren zum SIA451-Parser (frueher: Absturz, weil
            # alles als CRB-XML interpretiert wurde).
            if ext == "crb" and head_bytes[:2].isdigit():
                from .sia451 import Sia451Importer
                return Sia451Importer
            head_txt = head_bytes.lstrip()[:1]
            if not head_txt.startswith(b"<"):
                # weder ZIP noch XML -> Fixed-Width versuchen
                from .sia451 import Sia451Importer
                return Sia451Importer
            # sonst XML (SORBA/Messerli/Comatic) -> CRB-XML Parser
            from .crbx import CrbxImporter
            return CrbxImporter
