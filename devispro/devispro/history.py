...
class TrialLimitErreicht(Exception):
    """PREISMODELL v3: Gratis-Kontingent (5 Devis) aufgebraucht und weder
    Lizenz noch PPD-Entscheidung getroffen. Das GUI faengt diese Exception
    und zeigt den Modal-Dialog (Voll-Lizenz / Prepaid / Ehrenwort)."""
    pass


def _status_zaehlt(status: str) -> bool:
    """ENTSCHEIDUNG 2: 'Ein Devis' = erste finalisierte Speicherung.
    Entwürfe/Importe sind gratis; zaehlend sind finalisierte Offerten."""
    from devispro.trial_counter import ZAEHLende_STATUStextE
    return (status or "").lower() in ZAEHLende_STATUStextE
...
def save(devis, netto, name=None, method="mock", kanton="ZH", status="offen"):
    ...
    PREISMODELL v3 Hook (Konzept C.3): Vor dem Speichern eines
    finalisierten Devis darf_nutzen() pruefen. Bei Blockierung wird die
    TrialLimitErreicht-Exception geworfen (nichts wird gespeichert).
    """
    _ensure()
    if _status_zaehlt(status):
        try:
            from devispro import license as liz
            if not liz.darf_nutzen():
                raise TrialLimitErreicht(
                    "Gratis-Kontingent von 5 Devis ist aufgebraucht. "
                    "Bitte Voll-Lizenz, Prepaid-Paket oder Ehrenwort-PPD wählen.")
        except ImportError:
            pass  # trial_counter fehlt (alte Version) -> kein Zwang
    did = _next_id()
    ...
    try:
        ...
        pass
    # PREISMODELL v3: Zaehler + .dp_state-Marker aktualisieren; im PPD-
    # Prepaid-Modus Guthaben abbuchen (Ehrenwort laeuft ins Negative ->
    # status() meldet dann ppd_ueberzogen).
    try:
        from devispro import trial_counter as tc
        tc.aktualisiere(status)
        from devispro import license as liz
        if liz.modus() == "ppd" and _status_zaehlt(status):
            liz.ppd_devis_verbrauchen(1)
            # ERP-Zuschlag (Entscheidung 3): CHF 49 pro exportiertem Devis
            # bei PPD-Kunden mit ERP-Feature -> auf dem Devis vermerken,
            # Sammelrechnung liest das Feld aus.
            try:
                from devispro import abo as abomod
                if abomod.ist_erp():
                    meta["erp_zuschlag_chf"] = abomod.erp_zuschlag_chf()
