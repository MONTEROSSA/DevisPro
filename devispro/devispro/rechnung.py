...
class Rechnung:
    ...
    notiz: str = ""
    # --- Baurechnung-Erweiterung (Profi-Umfang) ---
    typ: str = "schluss"         # anzahlung | abschlag | schluss | gutschrift
    waehrung: str = "CHF"        # CHF | EUR
    anzahlung_pct: float = 0.0    # Akonto bei Auftragserteilung
    garantie_pct: float = 0.0    # Garantierückbehalt (typ. 5%)
    garantie_monate: int = 24    # Frist bis Freigabe
    zahlungsziel_tage: int = 30  # Tage ab Rechnungsdatum
    bereits_bezahlt: float = 0.0 # summe aller vorangeg. teilrechnungen (für schluss)
    referenz_nr: str = ""        # bezug zur offerte / leistungsvereinbarung
    ...
    # --- Baurechnung: Garantie & Anzahlung ---
    def garantie_betrag(self) -> float:
        return _r2(self.brutto() * self.garantie_pct / 100.0)

    def anzahlung_betrag(self) -> float:
        if self.typ == "anzahlung":
            return self.brutto()
        return _r2(self.brutto() * self.anzahlung_pct / 100.0)

    def offen_nach_anzahlung(self) -> float:
        return _r2(self.brutto() - self.anzahlung_betrag())

    def schluss_netto_auszahlung(self) -> float:
        """Brutto abzgl. Anzahlung (Akonto), bereits bezahlter Abschlaege und Garantie."""
        ausz = _r2(self.brutto() - self.anzahlung_betrag() - self.bereits_bezahlt - self.garantie_betrag())
        return ausz

    def faelliges_datum(self) -> str:
        try:
            from datetime import datetime, timedelta
            d = datetime.strptime(self.datum, "%Y-%m-%d") if "-" in self.datum else datetime.strptime(self.datum, "%d.%m.%Y")
            return (d + timedelta(days=self.zahlungsziel_tage)).strftime("%d.%m.%Y")
        except Exception:
            return self.faellig
...
def build_pdf(r: Rechnung, lang: str = "de") -> bytes:
    ...
    # Baurechnung: Anzahlung / Garantie / offen
    if r.typ in ("schluss",) and (r.anzahlung_pct or r.garantie_pct):
        extra = []
        if r.anzahlung_pct:
            extra.append(("Anzahlung (Akonto)", _chf(r.anzahlung_betrag())))
        if r.garantie_pct:
            extra.append((f"Garantierückbehalt {_chf(r.garantie_pct)}% "
                          f"(Frist {r.garantie_monate} Mon.)", "-" + _chf(r.garantie_betrag())))
        if r.bereits_bezahlt:
            extra.append(("Bereits bezahlt (Akonto/Abschläge)", "-" + _chf(r.bereits_bezahlt)))
        if extra:
            extra.append(("AUSZAHLUNG (offen, nach Garantie)", _chf(r.schluss_netto_auszahlung())))
            pdf.subtitle("Zahlungsdetails")
            pdf.summary([(k, v, i == len(extra) - 1) for i, (k, v) in enumerate(extra)])
    if r.zahlungsplan:
    ...
    if r.notiz:
        pdf.note(r.notiz)
    # Swiss QR-Code auf eigene (letzte) Seite, damit er nichts ueberlagert
    try:
        matrix, _ = qr_mod.qr_matrix_aus_rechnung(r)
        pdf._flush_page()                      # neue Seite erzwingen
        pdf.y = PN.PAGE_H - PN.MARGIN_T - 16
        pdf.ops.append((pdf.y, PN.MARGIN_L, "Zahlung (Swiss QR):", "F2", 10, "0 0 0"))
        pdf.y -= 10
        pdf.image(matrix, PN.MARGIN_L, PN.MARGIN_B + 6, 130)   # eigenstaendig links unten
    except Exception:
        pass
    return pdf.build()
