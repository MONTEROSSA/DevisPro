...
class PDF:
    ...
    def image(self, matrix, x, y, size, quiet=4):
        """Bet fet ein QR-Symbol (bool-Matrix) als FlateDecode-Grayscale-Bild ein.

        matrix: list[list[int]] (1=dunk, 0=hell), size: Kantenlaenge in pt.
        """
        n = len(matrix)
        dim = n + 2 * quiet
        raw = bytearray()
    ...
    def build(self):
        ...
        page_ids = []
        for (ops, lns, rects) in self.pages:
            content = self._render_content(ops, lns, rects, img_obj_ids)
            cid = next_id; next_id += 1
            ...
            if self._image_ops and img_obj_ids:
                parts = " ".join("%d %d 0 R" % (iid, img_obj_ids[iid]) for (_, _, _, _, iid) in self._image_ops if iid in img_obj_ids)
    ...
    def _render_content(self, ops, lns, rects, img_obj_ids=None):
        s = []
        if img_obj_ids and self._image_ops:
            for (x, y, w, h, iid) in self._image_ops:
                if iid in img_obj_ids:
                    s.append("q")
                    s.append("%.2f 0 0 %.2f %.2f %.2f cm" % (w, h, x, y))
                    s.append("%d 0 R Do" % img_obj_ids[iid])
                    s.append("Q")
