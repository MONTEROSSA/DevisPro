"""Web-UI für devispro (reine Python-Stdlib, kein Flask nötig).
...
import io
try:
    import cgi  # Py3.9-3.12
except ImportError:
    cgi = None  # Py3.13+ hat cgi entfernt
import html
import json
import time
import shutil
...
def _read_raw_post(self):
    """Rohdaten des POST-Bodys (für eigene Multipart-Parser)."""
...
def _parse_multipart_file(self, field_name):
    ...
    bei PNG-Bytes mit UnicodeDecodeError abstürzt). Liefert (filename, bytes)
    oder (None, None).
    """
...
<style>
 ...
 .vgl{{border-collapse:collapse;width:100%;margin-top:1rem;font-size:.9rem}}
 .vgl th,.vgl td{{border:1px solid #e2e2e2;padding:.5rem .65rem;text-align:left;vertical-align:top}}
 .vgl th{{background:#14532d;color:#fff;font-weight:700}}
 .vgl tr:nth-child(even) td{{background:#f8fafc}}
 .vgl .summe td{{background:#eef2ff;font-weight:700}}
 .vgl .small{{font-size:.78rem;color:#666}}
 .vgl .ok{{color:#15803d;font-weight:700}}
 .vgl .warn{{background:#fff7ed;color:#b45309;font-weight:600;font-size:.82rem}}
 .vgl-vorteile li{{margin:.45rem 0;line-height:1.5}}
 .btn{{margin-right:.6rem}}</style></head><body>
<script>
...
def render_nav(lang):
 ...
 <div class="brand"><a href="/" style="display:flex;align-items:center;gap:.5rem;text-decoration:none;color:#14532d"><img src="/static/devispro_logo.png" alt="DevisPro" style="height:34px;width:auto;border-radius:6px">devispro</a></div>
...
def render_agent_page(lang="de", did=None, history=None):
    """KI-Agent Seite; history = Liste von (role, text) Tuples, die im Chat-Log stehen."""
    import time as _t
    history = history or []
    profil = stammdaten.load_profile() or {}
    did = did or ""
    cache = int(_t.time() * 1000)
    devis_opts = "".join(
        f"<option value='{d['id']}' {'selected' if d['id']==did else ''}>{d['id']} – {html.escape(str(d.get('name','')))}</option>"
        for d in history_mod.list_all()[:50])
    kanton_anzeige = profil.get("kanton_label") or profil.get("kanton", "ZH")
    # chat-log inhalt
    if history:
        log_lines = "\n".join(
            ("👤 " + html.escape(m) if role == "user" else "🤖 " + m.replace("<", "&lt;").replace(">", "&gt;"))
            for role, m in history)
    else:
        log_lines = ("🤖 Hallo! Ich bin der DevisPro-KI-Agent. Was möchten Sie tun?\n"
            "Beispiele: «setze MWST auf 7.7», «exportiere nach Abacus», «rechne 5000 CHF in EUR um», «öffne devis_0007».\n"
            f"Aktueller Betrieb: {html.escape(str(profil.get('betrieb','')))} · Kanton: {html.escape(str(kanton_anzeige))} · MWST: {profil.get('mwst_pct',8.1)}%")
    body = (
        ...
        "und führt echte Aktionen aus: MWST/Kanton ändern, Export in Buchhaltung, Währung umrechnen, "
        "Marketing-Texte, Devis öffnen.</p>"
        f"<form method='get' action='/agent?_={cache}' style='margin-bottom:.6rem'><label>Betreff Devis: <select name='did'>"
        ...
        + log_lines +
        "</div>"
        f"<form method='post' action='/agent_chat?_={cache}' id='agentform' style='display:flex;gap:.5rem;margin-top:.6rem'>"
        f"<input type='hidden' name='did' value='{html.escape(str(did))}'>"
        ...
        "log.innerHTML+='\\n\\n👤 '+m;f.msg.value='';"
        "var fd=new FormData(f);fetch('/agent_chat?_='+Date.now(),{method:'post',body:new URLSearchParams(fd),headers:{'X-Requested-With':'fetch'},cache:'no-store'}).then(r=>r.text()).then(t=>{log.innerHTML+='\\n🤖 '+t;log.scrollTop=log.scrollHeight;});});</script>"
        "</div>"
    )
...
def render_devis_view(did, lang):
    ...
    # Margen-Copilot: Plausibilitätsprüfung
    warns = plausib_mod.check_positions(devis.positions)
    summ = plausib_mod.summarize(warns)
    if summ["ok"]:
        warn_box = f"<div class='save' style='margin-top:1rem'>✓ Margen-Copilot: keine Auffälligkeiten ({summ['label']}).</div>"
    else:
        items = "".join(
    ...
    try:
        ...
        if br["bewertet"] == 0:
            ...
            markt_box = (
                ...
                f"<span class='bm high'>▲ {len(br['ueber_markt'])} Pos. über Markt</span> "
                f"→ Auftrags-Risiko ~<b>{br['auftrags_risiko_chf']:,.0f} CHF</b>"
                f"<details style='margin-top:.5rem'><summary>Top-Positionen</summary>"
                f"<p class='meta'>Zu günstig (Marge verschenkt):</p><ul class='warn-list'>{u}</ul>"
                f"<p class='meta'>Zu teür (Zuschlag gefährdet):</p><ul class='warn-list'>{o}</ul></details>"
                f"</div>")
    except Exception:
        markt_box = ""
...
def _render_import_crbx(lang="de", msg=None, devis_id=None):
    """Upload-Formular fuer CRB-SIA (.crbx) Leistungsverzeichnisse."""
    note = ""
    if msg:
        note = f"<div class='okbox'>{html.escape(msg)}</div>"
    body = f"""
    <div class='card'>
      <h2>📥 CRB-SIA Leistungsverzeichnis importieren (.crbx)</h2>
      <p class='meta'>Lädt ein SIA-451-Leistungsverzeichnis (Messerli / SORBA / Comatic Submission)
      und erstellt daraus ein Devis. Positionen werden mit Ihren eigenen Preisen bepreist
      (falls hochgeladen), sonst mit CH-Durchschnittspreisen simuliert.</p>
      {note}
      <form method='post' action='/import_crbx' enctype='multipart/form-data'>
        <p><input type='file' name='crbx' accept='.crbx,.e1s,.sia' required></p>
        <p><button type='submit' class='btn'>Importieren</button></p>
      </form>
      <p class='meta'><a href='/preise_upload'>➡️ Eigene Preise hochladen (CSV)</a></p>
    </div>
    """
    if devis_id:
        body += f"<p class='meta'><a class='btn' href='/devis_view?id={devis_id}'>Importiertes Devis öffnen</a></p>"
    return render_page(body, lang)


def _render_preise_upload(lang="de", anz=0, msg=None):
    """Upload-Formular fuer eigene Firmenpreise (CSV)."""
    if msg is None and anz:
        msg = f"✓ {anz} Preise gespeichert. Diese werden beim nächsten .crbx-Import verwendet."
    note = f"<div class='okbox'>{html.escape(msg)}</div>" if msg else ""
    body = f"""
    <div class='card'>
      <h2>💰 Eigene Firmenpreise hochladen (CSV)</h2>
      <p class='meta'>Laden Sie Ihre eigenen Richtpreise hoch. Diese werden beim Import eines
      .crbx-Leistungsverzeichnisses verwendet — für eine echte Offerte mit Ihren Preisen.</p>
      <p class='meta'><b>CSV-Format (Trennzeichen ; oder ,):</b><br>
      Bezeichnung;Einheit;EP_CHF<br>
      Fenster aus Holz-Metall;m2;950<br>
      Montage;h;145<br>
      (optionale Spalte: BKP)</p>
      {note}
      <form method='post' action='/preise_upload' enctype='multipart/form-data'>
        <p><input type='file' name='preise' accept='.csv,.txt' required></p>
        <p><button type='submit' class='btn'>Preise speichern</button></p>
      </form>
      <p class='meta'><a href='/import_crbx'>⬅️ Zurück zum Devis-Import</a></p>
    </div>
    """
    return render_page(body, lang)
...
def render_index(self=None, lang=None):
 ...
 <div class="card">
  <form method="post" enctype="multipart/form-data" action="/import_devis">
   ...
   <p class="meta">Unterstützte Formate: <b>SIA-451/Sorba</b> (.sia), <b>Bauweb/Dädalus</b> (.csv),
   <b>generisches CSV/Excel</b> (.csv/.xlsx), <b>GÄB D84</b> (DE, .xml), <b>ÖNORM B 2063</b> (AT, .csv),
   ...
   <div class="card" style="background:#f0fdf4;border:1px solid #bbf7d0">
    ...
    <ul style="margin:.3rem 0 0 1.1rem;padding:0;line-height:1.6">
      ...
      <li><b>Gratis Devis-Check:</b> Kunden pruefen ein Devis in 30 Sek. anonym, ohne Login – und testen 3 Monate gratis.</li>
      <li><b>Alle Formate:</b> SIA-451/Sorba, Bauweb, CSV/Excel, GÄB (DE), ÖNORM (AT), XRechnung (EU) + Swiss QR-Rechnung.</li>
    </ul>
...
def render_preis_tabelle() -> str:
    ...
    Jede Zeile hat editierbare Felder (EP/Kategorie) + Speichern/Löschen."""
    ...
    return (
        "<p class='meta'>Unter <b>EP</b> und <b>Kategorie</b> direkt ändern, dann "
...
def render_result(rows, total, currency, review, count, file, review_positions, devis_id=None, lang="de"):
    ...
    if review > 0:
        ...
        block = (f"<div class='blocker'>🔒 <b>Download gesperrt:</b> {review} Position(en) mit unsicherem "
                 f"Match müssen zürst manüll bepreist werden.<ul>{rev_list}</ul>"
                 f"<p>Erst nach manüller Korrektur (oder bewusster Freigabe) darf die Datei für Sorba exportiert werden.</p>"
 ...
 <p><b>Ergebnis:</b> {count} Positionen · Gesamtbetrag (Netto) <b>{total:,.2f} {currency}</b> ·
    Manülle Prüfung nötig: <span class="{'rev' if review>0 else 'ok'}">{review}</span></p>
 {block}
 {dl}
 {saved_note}
 <table><thead><tr><th>Pos</th><th>Artikel</th><th>EP CHF</th><th>Menge</th><th>Betrag</th><th>Conf</th><th>Review</th></tr></thead><tbody>"""
    ...
    # --- Zusatzpositionen (Fachbetrieb ergänzt VOR-ORT-Erkenntnisse) ---
    try:
...
def _recompute_and_render(devis_path, edit_pos_nr=None, ep=None, menge=None):
    ...
    manüll geprüft (Conf 1.0, review=False) und rendert das Ergebnis neu.
    Bereits manüll geprüfte Positionen (samt editierter Werte) bleiben erhalten
    ...
    for p in devis.positions:
        if p.pos_nr == edit_pos_nr and (ep is not None or menge is not None):
            ...
            p.confidence = 1.0
            p.matched_artikel = p.matched_artikel or "manüll"
            p.begruendung = "Manüll vom Fachbetrieb korrigiert"
            p.fill()
        elif p.pos_nr in reviewed:
            # bereits manüll geprüft: editierte Werte aus SIA behalten, kanton anwenden
            p.ep = (p.ep or 0.0) * kf
            p.requires_review = False
            p.confidence = 1.0
            p.fill()
...
def render_admin(lang="de"):
 ...
 <div class="card">
  ...
  <p class="meta"><b>Format-Unterstützung:</b> DevisPro importiert SIA-451/Sorba, Bauweb/Dädalus,
  generisches CSV/Excel, GÄB D84 (DE), ÖNORM (AT) und XRechnung (EU) – und erzeugt pro Devis
  ...
  <form method="post" action="/admin_erinnerung" style="margin-top:.8rem">
   <button type="submit">Erinnerungen jetzt pruefen/versenden</button>
  </form>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send_nocache(self, body: bytes, ctype="text/html; charset=utf-8"):
        """Wie _send, aber mit No-Cache-Headern (Safari zeigt immer frische Seite)."""
        self.send_response(200)
        self.send_header("Content-Type", ctype)
        self.send_header("Content-Length", str(len(body)))
    ...
    def _fieldstorage(self):
        ...
        # Py3.13+: cgi weg, gib leere FieldStorage zurueck (kein File-Upload)
        return None

    def do_GET(self):
        lang = lang_from_request(self)
        parsed = urlparse(self.path)
        # --- Trial-Gate: ohne gültige Lizenz nur Trial-Anmeldung zugreifen ---
        ...
        if parsed.path == "/qr_rechnung":
            ...
            try:
                ...
                kreditor = {
                    ...
                    "plz_ort": profil.get("plz_ort") or "8000 Zürich",
                    "land": "CH",
                ...
                body = (f"<div class='card'><h3>Swiss QR-Rechnung – {html.escape(did)}</h3>"
                        ...
                        f"<p class='meta'>QR-Code-Verfügbar: {qr_mod.qr_verfuegbar()}</p>"
        ...
        if parsed.path == "/check" and self.command == "GET":
            ...
            if q.get("embed", ["0"])[0] == "1":
                # Eigenständiges, einbettbares Widget (für devispro.de Landing)
                from devispro import lead_magnet as lead_mod
                check_html = lead_mod.render_form(lang)
                self._send(check_html.encode("utf-8"), "text/html; charset=utf-8")
                return
        ...
        if parsed.path == "/download":
        ...
        elif parsed.path == "/erp_vorschau":
            from devispro import erp_ui as _eu
            self._send(render_page(_eu.render_erp_vorschau(), lang).encode("utf-8"),
                       "text/html; charset=utf-8")
            return
        elif parsed.path == "/hilfe":
            from devispro import anleitung as _anl
            self._send(render_page(_anl.render_anleitung_html(lang), lang).encode("utf-8"),
                       "text/html; charset=utf-8")
            return
        elif parsed.path == "/download_gate":
            self._send(render_download_gate().encode("utf-8"), "text/html; charset=utf-8")
            return
        elif parsed.path == "/static/devispro_logo.png":
            zpath = os.path.join(HERE, "static", "devispro_logo.png")
            ...
            return
        elif parsed.path == "/DevisPro_Mac.zip":
            # Direkter Download (intern; normaler Flow geht über /download_gate mit Lead-Erfassung)
            _serve_devispro_zip(self)
            return
        ...
        elif parsed.path == "/import_crbx":
            q = parse_qs(parsed.query)
            ok = q.get("ok", ["0"])[0] == "1"
            did = q.get("id", [None])[0]
            msg = f"✓ Importiert: {len(history_mod.list_all())} Devisse im Verlauf." if ok else None
            self._send_nocache(_render_import_crbx(lang, msg=msg, devis_id=did if ok else None).encode("utf-8"))
        elif parsed.path == "/preise_upload":
            from devispro import firmen_preise as fp_mod
            anz = len(fp_mod.laden()) if fp_mod.exists() else 0
            self._send_nocache(_render_preise_upload(lang, anz=anz).encode("utf-8"))
            ...
            if not os.path.exists(bex):
                self._send(render_page("<div class='blocker'>✗ Bitte zürst ein Devis bepreisen.</div>", lang).encode("utf-8"))
                return
            try:
                devis = crb.parse(bex)
                profil = stammdaten.load_profile() or {}
            ...
            try:
                ...
                if olang not in LANGS:
                    olang = lang
                as_pdf = q.get("pdf", ["0"])[0] in ("1", "on", "trü")
                if as_pdf:
                    # echtes PDF via dependency-freiem Generator (immer verfügbar)
            ...
            try:
                ...
                bedingt = q.get("bedingt", ["0"])[0] in ("1", "on", "trü")
                as_pdf = q.get("pdf", ["0"])[0] in ("1", "on", "trü")
                if as_pdf:
                    # echtes PDF via dependency-freiem Generator (immer verfügbar)
            ...
            body = (
                ...
                "Leistungsverzeichnis (.sia/.crb), Zusatzblätter (CSV/XLSX), GÄB (.xml), XRechnung (.xml), "
                ...
                "<p class='meta' style='margin-top:1rem'>Unterstützte Formate: SIA-451/Sorba, Bauweb/Dädalus, "
                "generisches CSV/Excel, GÄB (DE), ÖNORM (AT), XRechnung (EU). Bilder/PDF werden als «manüll» "
                "markiert (OCR auf diesem System nicht verfügbar).</p></div>"
            )
            self._send(render_page(body, lang).encode("utf-8"))
            return
        elif parsed.path in ("/währung", "/waehrung"):
            q = parse_qs(parsed.query)
            did = q.get("id", [None])[0]
            ...
            body = (
                ...
                "<label>ERP-URL<input name='url' placeholder='https://büro.local/api/v1/belege' style='width:280px'></label> "
            ...
            self._send_nocache(render_agent_page(lang, did, history=[]).encode("utf-8"))
            return
            ...
            rows = "".join(
                ...
                f"<td><a class='btn-sm' href='/backup_restore?name={quote(b['name'])}' onclick=\"return confirm('Wirklich wiederherstellen? Aktülle Daten werden überschrieben.')\">↺</a></td></tr>"
            ...
            gewerke = ["Baumeister","Gipser","Maler","Plattenleger","Elektro","Sanitär","Fenster/Türen","Schreiner","Heizung/Lüftung","Sonstiges"]
            ...
            if not (abo_mod.darf("connector_abacus") or abo_mod.darf("connector_proffix") or abo_mod.darf("alle_formate")):
                ...
                return
            # System-Auswahl anzeigen, wenn kein ziel gewählt
            if not ziel:
                sys_rows = ""
            ...
            # Kaufen ist im 1. Jahr schon guenstiger als Mieten (und gehoert einem)
            prod_blocks = ""
            for key in abo_mod.ANZEIGE_PRODUKTE:
                p = abo_mod._bundle(key)
                miete_jahr = p["miete_chf"] * 12
                kauf_jahr = p["einmal_chf"] + p["support_jahr"]
                sparen = max(0, miete_jahr - kauf_jahr)
                komp_html = ""
                if "komponenten" in p:
                    komp_html = "<ul class='komponenten'>"
                    for ck, cv in p["komponenten"].items():
                        kurz = f" – {cv.get('kurz','')}" if cv.get("kurz") else ""
                        komp_html += (f"<li><b>{html.escape(cv['name'])}</b>{html.escape(kurz)}: "
                                      f"{cv['einmal_chf']} CHF + {cv['support_jahr']}/Jahr "
                                      f"&nbsp;|&nbsp; Miete {cv['miete_chf']} CHF/Mon</li>")
                    komp_html += "</ul>"
                prod_blocks += (
                    f"<div class='tarif'><h3>{html.escape(p['name'])}</h3>"
                    f"<div class='preis'>{p['einmal_chf']} CHF</div>"
                    f"<div class='intervall'>{html.escape(p['einmal_intervall'])}</div>"
                    f"{komp_html}"
                    f"<p class='meta'>Oder monatlich mieten: <b>{p['miete_chf']} CHF/{html.escape(p['miete_intervall'])}</b>, "
                    f"jederzeit kündbar.<br><b>Sie sparen im 1. Jahr: {sparen} CHF</b> – "
                    f"Kaufen ist ab sofort günstiger und gehört dauerhaft Ihnen.</p></div>"
                )
            body = (f"<h1>Abo &amp; Tarife</h1>"
                    ...
                    f"<h2>Zwei Produkte – Einmalzahlung oder monatlich mieten</h2>"
                    f"<div class='tarife'>{prod_blocks}</div>"
                    f"<p class='meta'>Kaufen ist schon im 1. Jahr günstiger als Mieten und läuft danach ohne laufende Pflicht weiter. "
                    f"Mieten = tiefe Einstiegshürde, kein Besitz, aber teurer pro Jahr.</p>")
            self._send(render_page(body, lang).encode("utf-8"))
            return
        elif parsed.path == "/vergleich":
            # Ehrliche Eigenpositionierung. Kein Konkurrenz-Preisvergleich
            # (Kunde will keine BKP/Abacus-Preisdebatte) – nur eigene Preise +
            # kurzer, faktischer Markt-Kontext.
            body = ("""
<h1>DevisPro – automatisierte Bauberechnung</h1>
<div class="logo-hero"><img src="/static/devispro_logo.png" alt="DevisPro" style="height:120px;width:auto;border-radius:12px"></div>

<h2>Unsere Preise (transparent)</h2>
<table class="vgl">
<tr><th>Produkt</th><th>Kauf (einmalig + Support)</th><th>Miete / Monat</th><th>Kaufen im 1. J. guenstiger?</th></tr>
<tr><td><b>DevisPro</b> (SIA-451 Bepreisung)</td><td>2'400 + 990/Jahr<br><span class="small">Jahr 1 = 3'390, ab J2 nur 990</span></td><td>350 (4'200/Jahr)</td><td class="ok">Ja – spart 810/Jahr</td></tr>
<tr><td><b>ERP-Modul</b> (Abacus/Proffix-Sync + Mahnung + Teilrechnung + White-Label)</td><td>+1'900 + 490/Jahr<br><span class="small">Jahr 1 = +2'390, ab J2 nur 490</span></td><td>+248 (2'976/Jahr)</td><td class="ok">Ja – spart 586/Jahr</td></tr>
<tr class="summe"><td><b>DevisPro &amp; ERP</b> (Bundle)</td><td>4'300 + 1'480/Jahr<br><span class="small">Jahr 1 = 5'780, ab J2 nur 1'480</span></td><td>598 (7'176/Jahr)</td><td class="ok">Ja – spart 1'396/Jahr</td></tr>
</table>

<h2>Warum DevisPro (kein Marketing-Blabla)</h2>
<ul class="vgl-vorteile">
<li><b>Kaufen ist ab Jahr 1 guenstiger als Mieten</b> – und gehoert Ihnen danach. Kein Zwangabo, Mieten jederzeit kuendbar.</li>
<li><b>ERP-Modul ist ein Add-on, kein zweites ERP:</b> Es synchronisiert mit dem ERP, das Sie meist schon haben (Abacus/Proffix) – plus Mahnung, Teilrechnung und White-Label.</li>
<li><b>Schweiz-spezifisch:</b> SIA-451, Sorba, Bauweb, GAEB, OENORM, XRechnung, Swiss QR. Alles eingebaut.</li>
<li><b>Gratis-Devis-Check &amp; Margen-Copilot:</b> Funktionen, die reine Bepreisungstools nicht bieten.</li>
<li><b>Offline &amp; lokal:</b> Kein Cloud-Zwang, keine laufende Serverpflicht. Lizenz ueber Freischalt-Code.</li>
</ul>

<p class="meta"><b>Preislicher Rahmen:</b> DevisPro ist im Umfeld der ueblichen Schweizer Bau-Software platziert – nicht teurer als vergleichbare ERP-/Bepreisungs-Loesungen, und beim Kauf ab Jahr 1 guenstiger als Mieten.
Wir vergleichen nicht laufend mit Konkurrenten, sondern liefern ein faires, transparentes Modell.</p>
<p><a class="btn" href="/abo">Zu den Tarifen</a> <a class="btn" href="/download_gate">Jetzt Devis erstellen (Test)</a></p>
""")
            self._send(render_page(body, lang).encode("utf-8"))
            return
        elif parsed.path == "/admin_white_label":
            # nur für Admins sichtbar (gleicher Guard wie andere Admin-Routen)
            tok = self.headers.get("Cookie", "").split("adminsess=")[-1].split(";")[0]
            if not auth.session_gueltig(tok):
                self._send(render_admin_login(fehler="Bitte zürst einloggen.", lang=lang).encode("utf-8"))
                return
            try:
                ...
                crows = ""
                for e in codes:
                    status = "eingelöst" if e.get("eingelöst") else "frei"
                ...
                body = (f"<h1>White-Label &amp; Verbands-Lizenzen</h1>"
                        ...
                        f"<p class='meta'>Codes sind HMAC-signiert. Einlösen via abo.setze_tarif() – "
            ...
            body = (
                ...
                "ob Ihre Kalkulation unter oder über Markt liegt. Keine Kundennamen, keine Projekte.</p>"
                "<div class='kpi'>"
                ...
                f"{html.escape(str(kt))}</b>. Beim nächsten Devis sehen Sie ▼/●/▲ pro Position im Vergleich "
                "zu diesen Marktpreisen. Premium-Kunden sehen kanton-spezifische Benchmarks.</p>"
                "<div class='save'>✓ Das Netzwerk wächst mit jedem Devis – Ihr Vorteil gegenüber starren Editoren.</div>"
                "</div>"
            )
            self._send(render_page(body, lang).encode("utf-8"))
            return
            ...
            if vorl:
                for v in vorl:
                    rows += (
                        ...
                        f"<input type='hidden' name='name' value='{html.escape(str(v['name']))}'>"
                        f"<button type='submit' class='btn-sm alt' onclick=\"return confirm('Vorlage loeschen?')\">🗑</button></form>"
                        "</td></tr>"
                    )
            else:
            ...
            # Wiederkehrende Rechnungen (Verträge / Service-Abo)
            ...
            if vertraege:
                for v in vertraege:
                    ...
                    rows += (
                        ...
                        f"<td style='text-align:right'>{v.get('nächste', '')}</td>"
                        f"<td style='display:flex;gap:.3rem'>"
                        ...
                        f"<input type='hidden' name='name' value='{html.escape(str(v['name']))}'>"
                        f"<button type='submit' class='btn-sm alt' onclick=\"return confirm('Vertrag loeschen?')\">🗑</button></form>"
                        "</td></tr>"
                    )
            else:
            ...
            body = (
                ...
                "<option value='jährlich'>jährlich</option></select></label>"
            ...
            # Team-Sync: Rolle admin/büro
            tok = self.headers.get("Cookie", "").split("teamsess=")[-1].split(";")[0]
            ben = team_mod.token_gueltig(tok)
            if team_mod.rolle_von(ben) not in ("admin", "büro"):
                self._send(render_page(
            ...
            body = (
                ...
                "<p class='meta'>Bundle auswählen (vom Büro-PC exportiert). Überschreibt nur ältere Stände.</p>"
                ...
                "<p class='meta'>Im Büro einmal ein Token erzeugen; Aussendienst-PCs holen den Stand per Token "
                "vom Büro-PC (solange dieser läuft).</p>"
                ...
                "<button type='submit' class='btn-sm'>Vom Büro holen</button></form>"
                "</div>"
                "</div>"
            )
            self._send(render_page(body, lang).encode("utf-8"))
            ...
            tok = self.headers.get("Cookie", "").split("adminsess=")[-1].split(";")[0]
            if not auth.session_gueltig(tok):
                self._send(render_admin_login("Bitte zürst einloggen.").encode("utf-8"))
                return
            from devispro import lead_magnet as lead_mod
            ...
            body = (
                ...
                "<p class='meta' style='margin-top:1rem'>Qülle: data/trial_leads.log (JSON-Lines). "
            ...
            if new_lang not in LANGS:
                new_lang = "de"
            # zurück zur vorherigen Seite (Referer) oder Start
    ...
    def do_POST(self):
        parsed = urlparse(self.path)
        lang = lang_from_request(self)
        if parsed.path == "/download_gate":
            form = self._fieldstorage()
            email = (form.getvalue("email") or "").strip()
            firma = (form.getvalue("firma") or "").strip()
            kanton = (form.getvalue("kanton") or "").strip()
            gewerk = (form.getvalue("gewerk") or "").strip()
            tarif = (form.getvalue("tarif") or "devis").strip()
            if tarif not in ("devis", "erp"):
                tarif = "devis"
            if not email or "@" not in email:
                self._send(render_download_gate(
                    fehler="Bitte eine gültige E-Mail eingeben – sonst können wir Ihnen den Pilot-Code nicht zustellen.").encode("utf-8"),
                    "text/html; charset=utf-8")
                return
            try:
                from devispro import lead_magnet as lead_mod
                lead_mod.log_download_lead(email, firma, kanton, gewerk, tarif=tarif)
            except Exception:
                pass
            _serve_devispro_zip(self)
            return
        ...
        if parsed.path == "/check_lead":
            ...
            try:
                ...
                pass
            # 1-Klick: E-Mail + Projekt ins Trial-Formular vorausfüllen
        ...
        if parsed.path == "/portal_import":
            ...
            try:
                ...
                os.remove(tmp)
                self.send_response(303)
                self.send_header("Location", f"/devis/{did}?importiert=1&qülle=portal")
                self.end_headers()
                return
            except Exception as e:
                try:
            ...
            try:
                ...
                if meta.get("ocr"):
                    ...
                    form_html = (
                        ...
                        "Positionen manüll – das Devis wird sofort bepreist.</p>"
        ...
        if self.path == "/agent_chat":
            try:
                ...
                if res.get("navigate"):
                    ans += f"\n\n\u2192 {res['navigate']}"
                # JS-Request (fetch) -> nur text; sonst volle seite mit chat-verlauf
                if self.headers.get("X-Requested-With") == "fetch":
                    ...
                    history = [("user", msg), ("bot", ans)]
                    self._send_nocache(render_agent_page(lang, did, history=history).encode("utf-8"))
            except Exception as e:
                # niemals den server crashen lassen
                err = f"⚠️ Agent-Fehler: {html.escape(str(e))}"
                if self.headers.get("X-Requested-With") == "fetch":
                    self.send_response(200)
                    self.send_header("Content-Type", "text/plain; charset=utf-8")
                    self.end_headers()
                    ...
                    self._send_nocache(render_agent_page(lang, did if 'did' in dir() else None,
                        history=[("user", msg if 'msg' in dir() else ""), ("bot", err)]).encode("utf-8"))
            return
        if self.path == "/backup_create":
            ...
            return
        if self.path == "/import_crbx":
            form = self._fieldstorage()
            up = form["crbx"] if "crbx" in form else None
            if up is None or not getattr(up, "filename", ""):
                self._send(render_page("<div class='blocker'>✗ Bitte eine .crbx-Datei wählen.</div>", lang).encode("utf-8"))
                return
            data = up.file.read()
            if not data:
                self._send(render_page("<div class='blocker'>✗ Datei ist leer.</div>", lang).encode("utf-8"))
                return
            tmp = os.path.join(DATA, "_upload_" + os.path.basename(up.filename))
            open(tmp, "wb").write(data)
            try:
                from devispro.importers import import_devis
                devis = import_devis(tmp)
                if not devis.positions:
                    self._send(render_page("<div class='blocker'>✗ Keine Positionen in der Datei gefunden.</div>", lang).encode("utf-8"))
                    return
                profil = stammdaten.load_profile() or {}
                kanton = profil.get("kanton", "ZH")
                did = history_mod.save(devis, 0.0, name=devis.meta.get("projekt", up.filename),
                                       method="crbx", kanton=kanton, status="importiert")
                os.remove(tmp)
                self.send_response(303)
                self.send_header("Location", f"/import_crbx?ok=1&id={did}")
                self.end_headers()
            except Exception as e:
                self._send(render_page(f"<div class='blocker'>✗ Import fehlgeschlagen: {html.escape(str(e))}</div>", lang).encode("utf-8"))
            return
        if self.path == "/preise_upload":
            form = self._fieldstorage()
            up = form["preise"] if "preise" in form else None
            if up is None or not getattr(up, "filename", ""):
                self._send(render_page("<div class='blocker'>✗ Bitte eine CSV-Datei mit Preisen wählen.</div>", lang).encode("utf-8"))
                return
            tmp = os.path.join(DATA, "_preise_upload.csv")
            open(tmp, "wb").write(up.file.read())
            try:
                from devispro import firmen_preise as fp_mod
                anz = fp_mod.speichern_aus_upload(tmp)
                os.remove(tmp)
                self.send_response(303)
                self.send_header("Location", f"/preise_upload?ok=1&n={anz}")
                self.end_headers()
            except Exception as e:
                self._send(render_page(f"<div class='blocker'>✗ Fehler: {html.escape(str(e))}</div>", lang).encode("utf-8"))
            return
        ...
        if self.path == "/trial_anmelden":
            ...
            if res["ok"]:
                # Nach Anmeldung direkt in die App (Lizenz ist jetzt 90 Tage gültig)
                self.send_response(303)
        ...
        if self.path == "/admin_smtp":
            ...
            if preset == "hostinger":
                ...
                body = ("<div class='okbox'>✓ Hostinger (devispro.de) gespeichert. "
                        "Mails werden jetzt über smtp.hostinger.com versendet.</div>" if ok
        ...
        if self.path == "/admin_smtp_test":
            ...
            if not to:
                ...
                body = "<div class='blocker'>Kein SMTP konfiguriert. Zürst Hostinger/SMTP speichern.</div>"
            else:
                ...
                body = ("<div class='okbox'>✓ Test-Mail an %s gesendet.</div>" % to if ok
                        else "<div class='blocker'>Versand fehlgeschlagen – Host/Passwort pruefen.</div>")
            self._send(render_page(
                ...
                if not empfaenger:
                    ...
                    for e in empfaenger:
                        ...
                        if not ok:
                            fehler.append(e)
                    if fehler:
                        body = ("<div class='blocker'>Versand an %s fehlgeschlagen – SMTP pruefen.</div>"
                                % ", ".join(fehler))
                    else:
        ...
        if self.path == "/team_del":
            ...
            ben = team_mod.token_gueltig(tok)
            if team_mod.rolle_von(ben) != "admin":
                self._send(render_page("<div class='blocker'>🔒 Nur Admin darf Benutzer loeschen.</div>", lang).encode("utf-8"))
                return
            form = self._fieldstorage()
        ...
        if self.path == "/sync_import":
            ...
            try:
                ...
                detail = (f"Aktualisiert: {len(rep['updated'])} · Neu: {len(rep['added'])} · "
                          f"Beibehalten (neüste): {len(rep['skipped'])}")
        ...
        if self.path == "/sync_token":
            ...
            body = (f"<div class='okbox'>✓ LAN-Token erzeugt: <code>{html.escape(t)}</code><br>"
                    ...
                    "<code>sync_token.json</code> im Büro besteht).</div>"
                    f"<a class='btn' href='/sync'>← Zurück zu Team-Sync</a>")
            self._send(render_page(body, lang).encode("utf-8"))
            return
        if self.path == "/sync_pull":
            ...
            try:
                from urllib.reqüst import urlopen, Reqüst
                req = Reqüst(url.rstrip("/") + "/sync_bundle?token=" + token)
                ...
                body = (f"<div class='okbox'>✓ Vom Büro geholt & angewendet. {html.escape(detail)}</div>"
                        f"<a class='btn' href='/sync'>← Zurück zu Team-Sync</a>")
                self._send(render_page(body, lang).encode("utf-8"))
            except Exception as e:
        ...
        if self.path == "/wr_anlegen":
            ...
            try:
                ...
                body = (f"<div class='okbox'>✓ Vertrag «{html.escape(name)}» angelegt. "
                        f"Nächste Rechnung faellig: <b>{html.escape(str(v['nächste']))}</b>. "
        ...
        if self.path == "/wr_rechnung":
            ...
            try:
                ...
                body = (
                    ...
                    f"· Fällig: {html.escape(res['faellig'])} · Nächste: {html.escape(res['nächste'])}</p>"
        ...
        if self.path == "/override":
            # Fachkraft-Freigabe: nur Rollen admin/büro dürfen den Review-Blocker aufheben
            tok = self.headers.get("Cookie", "").split("teamsess=")[-1].split(";")[0]
        ...
        if self.path == "/process_extras":
            ...
            if own_bez:
                try:
                    op = float(form.getvalue("own_ep") or "0")
                    omg = float(form.getvalue("own_mg") or "0")
                except Exception:
                    op = omg = 0.0
                obet = op * omg
                extra_sum += obet
                extras_rows.append((own_bez, form.getvalue("own_eh", ""), op, omg, obet))
            ...
            # Zusatzpositionen als dicts für SIA-Export aufbereiten
            extras_dicts = []
 ...
 <div class="card">
  ...
  <a class="btn" href="/" style="margin-left:1rem">⬅ Neues Devis</a>
        ...
        if self.path == "/erp_aktion":
            ...
            try:
                if aktion == "wareneingang" and nr:
                    ...
                    meld = f"Wareneingang {wert} für {nr} erfasst."
                    ...
                    if pnr and positionen:
                        ...
                        meld = f"Beleg ({btyp}) für {pnr} erstellt."
                    else:
                        meld = "Fehler: Partner-Nr und mind. 1 Position nötig."
            except Exception as e:
                meld = f"Fehler: {e}"
            self.send_response(303)
        ...
        crb.export(devis, os.path.join(DATA, out_name))
        total = sum((p.betrag or 0.0) for p in devis.positions)
        # Verlauf speichern (jedes Devis bleibt später ansehbar)
        try:
...
def _serve_devispro_zip(handler):
    """Echte DevisPro_Mac.zip aus dem Root-Verzeichnis ausliefern."""
    zpath = os.path.join(HERE, "DevisPro_Mac.zip")
    if os.path.exists(zpath):
        with open(zpath, "rb") as f:
            data = f.read()
        handler.send_response(200)
        handler.send_header("Content-Type", "application/zip")
        handler.send_header("Content-Disposition", 'attachment; filename="DevisPro_Mac.zip"')
        handler.send_header("Content-Length", str(len(data)))
        handler.end_headers()
        handler.wfile.write(data)
        return
    handler._send("Download nicht verfügbar.".encode("utf-8"), "text/plain; charset=utf-8")


def render_download_gate(fehler="", email="", firma="", kanton="", gewerk="", lang="de", tarif="devis"):
    """Lead-Gate vor dem Software-Download: E-Mail + Firma Pflicht, damit der
    Pilot-Code zugestellt werden kann. Erfasst den Lead und liefert dann die Zip.
    tarif: 'devis' oder 'erp' – der Kunde waehlt vor dem Download, was er testen moechte."""
    kanton_opts = "".join(
        f'<option value="{k}"{" selected" if kanton == k else ""}>{k} – {lbl}</option>'
        for k, lbl in KANTON_CHOICES)
    err = f'<div class="blocker">⚠ {html.escape(fehler)}</div>' if fehler else ""
    body = f"""
<div class="card" style="max-width:560px;margin:2rem auto">
 <h2>⬇ DevisPro herunterladen</h2>
 <p class="meta">Geben Sie Ihre Daten ein – wir senden Ihnen den <b>3-Monate-Pilot-Code</b>
 per E-Mail und Sie erhalten die Software sofort. Kein Spam, DSGVO-konform.</p>
 {err}
 <form method="post" action="/download_gate">
  <div class="grid">
   <div><label>E-Mail *</label><input type="email" name="email" required value="{html.escape(email)}" placeholder="name@firma.ch"></div>
   <div><label>Firma *</label><input type="text" name="firma" required value="{html.escape(firma)}" placeholder="Ihr Betrieb AG"></div>
   <div><label>Kanton</label><select name="kanton">{kanton_opts}</select></div>
   <div><label>Gewerk</label><input type="text" name="gewerk" value="{html.escape(gewerk)}" placeholder="z.B. Maler"></div>
  </div>
  <fieldset style="margin-top:1rem;border:1px solid #cfe3d6;border-radius:8px;padding:.8rem 1rem">
   <legend><b>Was möchten Sie 3 Monate kostenlos testen?</b></legend>
   <label style="display:block;margin:.3rem 0"><input type="radio" name="tarif" value="devis"{" checked" if tarif!="erp" else ""}> <b>DevisPro</b> – Devis automatisch bepreisen<br><span class="meta">Kauf 2'400 CHF + 990 CHF/Jahr &middot; <b>Miete 350 CHF/Mon</b> (jederzeit kündbar)</span></label>
   <label style="display:block;margin:.3rem 0"><input type="radio" name="tarif" value="erp"{" checked" if tarif=="erp" else ""}> <b>DevisPro + ERP</b> – inkl. Lager, Einkauf, Verkauf, Buchhaltung, Dashboard<br><span class="meta">Kauf 4'300 CHF + 1'480 CHF/Jahr &middot; <b>Miete 598 CHF/Mon</b> (jederzeit kündbar)</span></label>
   <p class="meta" style="margin:.4rem 0 0">Sie können später jederzeit auf das volle ERP upgraden – <b>ohne dass Ihre Daten oder berechneten Devis verloren gehen</b>. Beim Kauf wird nur der Freischalt-Code ergänzt; alles Bleibt erhalten. <b>Kaufen ist im 1. Jahr schon günstiger als Mieten</b> und gehört dauerhaft Ihnen.</p>
  </fieldset>
  <button type="submit" class="btn" style="margin-top:1rem">⬇ Jetzt herunterladen &amp; Pilot-Code erhalten</button>
 </form>
 <p class="meta">Schon Daten eingegeben? <a href="/DevisPro_Mac.zip">Direkt zur Datei</a></p>
</div>
"""
    return render_page(body, lang)
...
if __name__ == "__main__":
    # Markt-Benchmark beim ersten Start mit realistischen CH-Werten befüllen
    try:
        ...
        pass
    print(f"devispro UI läuft auf http://localhost:{PORT}")
